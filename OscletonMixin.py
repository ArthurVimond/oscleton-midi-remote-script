...
class OscletonMixin:
    ...
    @staticmethod
    def set_log(func):
        OscletonMixin.log_message = func

    @staticmethod
    def set_osc_server(handler):
        OscletonMixin._osc_server = handler

    @staticmethod
    def release_attributes():
        OscletonMixin._osc_server = None
        OscletonMixin.log_message = None
    ...
    def send(self, addr, *msg):
        if self._is_enabled_ovr:
            self._osc_server.send(addr, tuple(msg))

    def sendb(self, bundle):
        self._osc_server.send_message(bundle)

    def add_callback(self, address, func):
        """ Add a callback for an osc message """
        if self._osc_server:
            self._osc_server.add_handler(address, func)
    ...
    def add_simple_callback(self, addr, obj, property, check, fn):
        ...
        def cb(msg):
            self.log_message('add_simple_callback - cb() - msg: ' + str(msg))
            if check(msg):
                if len(msg) == len(self._default_args) + 2 or (
                        len(msg) == len(self._default_args) + 3 and msg[-1] == 'query'):
                    fn()
                else:
    ...
    def add_listener(self, addr, property, obj, value, *a):
        ...
        fn = wraps(fn)(mixin(SubjectSlot, CallableSlotMixin)(event=property, listener=partial(fn, self),
    ...
    def has_arg(self, msg):
        return not (len(msg) == 1 or (len(msg) == 2 and msg[1] == 'query'))
