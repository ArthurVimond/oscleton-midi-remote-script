...
class OscletonTransportComponent(TransportComponent, OscletonMixin):

    def __init__(self, *a, **kw):
        super(OscletonTransportComponent, self).__init__(*a, **kw)
    ...
    def _add_return_track(self, msg):
        self.song().create_return_track()

    def _add_scene(self, msg):
        self.song().create_scene(msg[2] if len(msg) == 3 else 0)

    def _del_track(self, msg):
        if len(msg) == 3:
            self.song().delete_track(msg[2])

    def _del_return_track(self, msg):
        if len(msg) == 3:
            self.song().delete_return_track(msg[2])

    def _del_scene(self, msg):
        if len(msg) == 3:
            self.song().delete_scene(msg[2])

    def _add_clip(self, msg):
        if len(msg) >= 4:
